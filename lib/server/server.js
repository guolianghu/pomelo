...
var TaskManager = require('../common/manager/taskManager' ).TaskManager;
var taskManager = new TaskManager();
taskManager.timeout = 5000;
...
//	console.log("this.app.enable('handleSerial')", this.app.enabled('handleSerial'));
	this.handle = initHandle(this.app.enabled('handleSerial'))
...
function initHandle(serial){
	if (!serial) {
		return handleMsg;
	}
	return addQueueHandle;
}
 ...
 * @param cb
 */
function addQueueHandle(msg, session, cb){
	var self = this;
	taskManager.addTask(session.id, function(task){
		handleMsg.call(self, msg, session, function(){
			task.done();
			cb.apply(cb, arguments);
		});
	}, function(){
		cb(new Error('request handle timeout'));
		logger.warn('request handle timeout', msg, session.id);
	})
}
...
function handleMsg(msg, session, cb) {
...
//	if (!session.mockLocalSession) {
//		var err = new Error('test');
//		console.log(err.stack);
//	}

	if(this.app.getServerType() !== routeRecord.serverType) {
		doForward(this.app, msg, session.mockLocalSession ? session.mockLocalSession() : session, routeRecord, cb);
	} else {
		doHandle(this, msg, session.mockLocalSession ? session.mockLocalSession() : session, routeRecord,cb);
	}
};
